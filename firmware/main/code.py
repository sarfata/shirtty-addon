...
from lib.ir.ir import get_pulses, get_code
...
def handle_pulses(pulses):
    if pulses:
        nerf_blast = Pulses.nerf.match(pulses)
        if nerf_blast == Pulses.kind.NERF_TEAM_1:
            set_color_hsl(1, 1, 0.5)
            time.sleep(2)
            return
        elif nerf_blast == Pulses.kind.NERF_TEAM_2:
            set_color_hsl(3, 1, 0.5)
            time.sleep(2)
            return

        # Not a nerf blast - maybe a IR command
        code = get_code(pulses)
        if code is not None:    
